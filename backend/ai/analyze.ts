...
export const analyzeTranscript = api<AnalyzeTranscriptRequest, AnalyzeTranscriptResponse>(
  ...
  async (req) => {
    try {
      ...
      const cacheKey = `analyze:${req.analysisType}:${hashString(req.transcript.trim())}`;
      const cached = await getCached<AnalyzeTranscriptResponse>(cacheKey);
      ...
      switch (req.analysisType) {
        case "sentiment":
          systemPrompt =
            "You are an expert at analyzing sentiment in conversations and meetings. Analyze the overall sentiment, confidence level, and provide detailed insights. Only return a valid JSON object. No prose.";
...
Transcript: ${req.transcript}`;
        ...
        case "topics":
          systemPrompt =
            "You are an expert at identifying and categorizing topics in conversations. Extract the main topics discussed and their relevance. Only return a valid JSON object. No prose.";
...
Transcript: ${req.transcript}`;
        ...
        case "speakers":
          systemPrompt =
            "You are an expert at identifying speakers and their contributions in conversations. Analyze speaker patterns and key points. Only return a valid JSON object. No prose.";
...
Transcript: ${req.transcript}`;
        ...
        case "action_items":
          systemPrompt =
            "You are an expert at extracting action items, tasks, and commitments from meeting transcripts. Only return a valid JSON object. No prose.";
...
Transcript: ${req.transcript}`;
        ...
        case "summary_insights":
          systemPrompt =
            "You are an expert at extracting key insights, decisions, and important information from meeting transcripts. Only return a valid JSON object. No prose.";
...
Transcript: ${req.transcript}`;
      ...
      const response: AnalyzeTranscriptResponse = { analysis };
      await setCached(cacheKey, response, { ttlSeconds: 60 * 60 * 24 * 3 }); // 3 days
