...
import { openAIChat, hashString, concurrentMap } from "./utils";
...
export const advancedAnalysis = api<AdvancedAnalysisRequest, AdvancedAnalysisResponse>(
  ...
  async (req) => {
    ...
    try {
      ...
      const typesKey = [...req.analysisTypes].sort().join(",");
      const customKey = req.customPrompts?.length ? hashString(JSON.stringify(req.customPrompts)) : "none";
      const cacheKey = `adv:${typesKey}:${customKey}:${hashString(req.transcript.trim())}`;

      const cached = await getCached<AdvancedAnalysisResponse>(cacheKey);
      if (cached) {
        return cached;
      }
      ...
      // Process analysis types concurrently with a sensible concurrency limit
      if (req.analysisTypes.length > 0) {
        const entries = await concurrentMap(req.analysisTypes, 3, async (analysisType) => {
          try {
      ...
      // Process custom prompts concurrently
      if (req.customPrompts && req.customPrompts.length > 0) {
        const custom = await concurrentMap(req.customPrompts, 3, async (prompt) => {
          try {
            return await performCustomAnalysis(req.transcript, prompt);
          } catch (error) {
            console.warn(`Failed to perform custom analysis "${prompt.name}":`, error);
            return null;
          }
        });
        analysis.customAnalysis = custom.filter((c) => c !== null);
      }
      ...
      // Cache for 1 day
      await setCached(cacheKey, response, { ttlSeconds: 60 * 60 * 24 });
...
async function performCustomAnalysis(transcript: string, customPrompt: { name: string; prompt: string }) {
  ...
  return {
    ...
    result: content || "No result generated",
  };
}
    ...
    [
      {
        role: "system",
        content:
          "You are an expert at synthesizing meeting analysis into actionable insights. Generate comprehensive insights including key takeaways, next steps, follow-up meetings, and stakeholder actions. Only return valid JSON without any extra text.",
      },
      {
        role: "user",
