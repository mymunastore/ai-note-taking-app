  ...
  async (req) => {
    try {
      ...
      if (req.participants && req.participants.length > 0) {
        const participantConditions = req.participants
          .map(() => {
      ...
      if (req.topics && req.topics.length > 0) {
        const topicConditions = req.topics
          .map(() => {
        ...
        [
          {
            role: "system",
            content:
              "You are an expert meeting facilitator and organizational consultant. Generate comprehensive meeting templates that include detailed agendas, preparation guidelines, and best practices. Return only valid JSON.",
          },
          {
            role: "user",
      ...
      [
        {
          role: "system",
          content:
            "You are an expert meeting facilitator and agenda planner. Generate comprehensive agendas with detailed outcomes, prerequisites, and dependencies. Focus on actionable items, clear outcomes, and efficient time management. Return only valid JSON.",
        },
        {
          role: "user",
      ...
      [
        {
          role: "system",
          content:
            "You are an expert executive assistant creating comprehensive briefing documents with risk assessment and stakeholder analysis. Analyze previous meetings to provide context, identify patterns, and highlight important information for upcoming meetings. Return only valid JSON.",
        },
        {
          role: "user",
      ...
      [
        {
          role: "system",
          content:
            "You are an expert meeting facilitator. Generate thoughtful discussion points with success criteria and fallback options that will drive productive conversations based on previous meeting context. Focus on unresolved issues, follow-ups, and strategic discussions. Return only valid JSON.",
        },
        {
          role: "user",
...
[
  {
    ...
    "successCriteria": ["criteria1", "criteria2"]
      ...
      { model: "gpt-4o", temperature: 0.4, max_tokens: 2500 }
    );
      ...
      [
        {
          role: "system",
          content:
            "You are an expert meeting consultant. Provide strategic recommendations with detailed metrics, follow-up actions, and challenge mitigation for meeting structure, time management, and success metrics based on previous meeting patterns and best practices. Return only valid JSON.",
        },
        {
          role: "user",
      ...
      [
        {
          role: "system",
          content:
            "You are an expert in meeting formats and facilitation techniques. Suggest alternative meeting formats that might be more effective for the given context. Return only valid JSON.",
        },
        {
          role: "user",
...
[
  {
    ...
    "suitability": 0.0_to_1.0_score
      ...
      { model: "gpt-4o", temperature: 0.4, max_tokens: 1500 }
    );
...
async function generateMeetingInsights(meetings: any[], analysisDepth: string) {
  const totalDuration = meetings.reduce((sum: number, m: any) => sum + m.duration, 0);
  const averageDuration = meetings.length > 0 ? totalDuration / meetings.length : 0;

  const meetingTypes = meetings.map((m: any) => {
  ...
  const typeFrequency = meetingTypes.reduce((acc: Record<string, number>, type: string) => {
  ...
  const meetingHours = meetings.map((m: any) => new Date(m.created_at).getHours());
  const hourFrequency = meetingHours.reduce((acc: Record<number, number>, hour: number) => {
  ...
  patterns.forEach((pattern) => {
    if (pattern.successRate < 0.7) {
      recommendations.push(
        `Consider restructuring meetings with pattern: "${pattern.pattern}" (${Math.round(pattern.successRate * 100)}% success rate)`
      );
    }
      ...
      [
        {
          role: "system",
          content:
            "You are an expert predictive analyst. Based on meeting patterns and trends, predict future challenges and opportunities. Return only valid JSON.",
        },
        {
          role: "user",
