...
export const chat = api<ChatRequest, ChatResponse>(
  ...
  async (req) => {
    try {
      // Try Cohere first for better conversation handling
      try {
...
${req.context ? `\n\nContext from user's notes/recordings:\n${req.context}` : ''}`;
        ...
        const response = await fetch("https://api.cohere.ai/v1/chat", {
          ...
          body: JSON.stringify({
            model: "command-r",
            message: req.message,
            chat_history: req.chatHistory?.map(msg => ({
              role: msg.role,
              message: msg.message
            })) || [],
            preamble: preamble,
        ...
        console.warn("Cohere chat failed, falling back to OpenAI:", cohereError);
      }
